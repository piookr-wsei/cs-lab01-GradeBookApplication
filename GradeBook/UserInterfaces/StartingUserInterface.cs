...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var type = parts[2];
            var isWeighed = AssessBool(parts[3]);

            

            IGradeBook gradeBook = null;

            if (type == "standard")
            {
                gradeBook = new StandardGradeBook(name, isWeighed);
            }
            else if(type == "ranked")
            {
                gradeBook = new RankedGradeBook(name, isWeighed);
            }
            else
            {
        ...
        private static bool AssessBool(string value)
        {
            if(value == "true")
            {
                return true;
